...
/// 结构化设计系统全景交互式预览展厅（Structured 标杆级风格）
...
public struct DesignSystemGalleryView: View {
    ...
    @State private var streamingText = "正在基于 Structured 范式生成黄金 3 秒开场白：\n“90% 的创作者都做错了第一步，真正的高完播率其实藏在这三个细节里……”"
        ...
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
            ...
            HeroBannerSheet(
                ...
                VStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
                    Text("浮岛卡片内容嵌入")
                        .font(DesignSystem.Typography.headline)
                    Text("完美复刻 Structured 标志性彩色顶栏，包含磨砂关闭键、白色超椭圆图标徽章与步骤标题。")
                        .font(DesignSystem.Typography.body)
                        .foregroundColor(DesignSystem.Color.textSecondary)
                }
