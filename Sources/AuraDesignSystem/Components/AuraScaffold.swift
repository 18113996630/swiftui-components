...
/// 1. 【页面统一律动】：页面级业务视图必须首选 `AuraScaffold` 作为根容器，禁止自由拼接杂乱外边距；
...
/// ```swift
/// AuraScaffold {
///     AuraSection(title: "个人看板", icon: "person.crop.circle") {
///         BaseCard {
...
/// }
/// ```
public struct AuraScaffold<Content: View>: View {
...
@available(*, deprecated, renamed: "AuraScaffold")
public typealias StructuredScaffold = AuraScaffold

#Preview("AuraScaffold Demo") {
    AuraScaffold {
        AuraSection(title: "核心参数", icon: "slider.horizontal.3", badgeText: "已保存") {
            BaseCard {
                Text("浮岛卡片内容 1")
        ...
        AuraSection(title: "分发渠道", icon: "arrow.triangle.branch") {
            BaseCard {
                Text("浮岛卡片内容 2")
