...
/// Aura 标杆级页面脚手架全景预览视图（AuraScaffold & AuraSection Demo）
///
/// 演示标准页面容器（AuraScaffold）与段落分组（AuraSection）的协同装配范式，
/// 包含 16pt 外边距、24pt 段落呼吸节奏、纯白浮岛卡片（BaseCard）承托与多巴胺主题穿透换肤。
///
/// ⚠️ 设计系统红线（Design Guardrails）：
/// 1. 【顶层脚手架】：整页必须以 `AuraScaffold` 为根视图；
/// 2. 【分段规范】：各业务模块以 `AuraSection` 分割，自动遵循 24pt 纵向呼吸节奏；
